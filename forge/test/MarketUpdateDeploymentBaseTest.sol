...
abstract contract MarketUpdateDeploymentBaseTest is Test {
    ...
    function createMarketUpdateDeployment(
        Vm vm
    ) public returns (MarketUpdateContractsDeployer.DeployedContracts memory) {
        ...
        MarketUpdateContractsDeployer.DeployedContracts memory deployedContracts = MarketUpdateContractsDeployer
            .deployContracts(
                vm,
                salt,
    ...
    function createMarketUpdateDeploymentForL2(
        Vm vm,
        ...
        if (chain == ChainAddresses.Chain.OPTIMISM) {
            ...
            return
                MarketUpdateContractsDeployer.DeployedContracts({
        ...
        GovernanceHelper.ProposalRequest memory proposalRequest = GovernanceHelper.createDeploymentProposalRequest(
            addresses
        );
    ...
    function updateAndVerifySupplyKink(
        ...
        assertEq(
            MarketAdminPermissionChecker(deployedContracts.marketAdminPermissionChecker).marketAdmin(),
            deployedContracts.marketUpdateTimelock
        );
        ...
        string memory description = string(
            abi.encodePacked("Proposal to update Supply Kink for ", marketName, " Market by Governor Timelock")
        );
        ...
        description = string(
            abi.encodePacked("Proposal to update Supply Kink for ", marketName, " Market by Market Admin")
        );
        GovernanceHelper.createAndPassMarketUpdateProposal(
            vm,
            chainAddresses.marketAdmin,
            proposalRequest,
            description,
    ...
    function updateAndVerifySupplyKinkInL2(
        ...
        GovernanceHelper.createAndPassMarketUpdateProposalL2(
            vm,
            chainAddresses.marketAdmin,
            proposalRequest,
            description,
